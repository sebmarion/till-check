#!/usr/bin/env node
if (process.env.TILL_TEST_PORT)
  throw new Error(
    "External-server write tests are disabled. Unset TILL_TEST_PORT; this suite must create its own disposable database.",
  );
...
async function startOwnServer() {
  tmpDbPath = path.join(
    os.tmpdir(),
    `till-e2e-${Date.now()}-${process.pid}.sqlite`,
  );
  ...
  throw new Error(
    `own test server did not become healthy on port ${TEST_PORT}`,
  );
}
...
const BASE_OFFSET =
  process.env.TILL_TEST_DATE_OFFSET !== undefined
...
serialTest(
  "POST /entry records card-extra-for-cash transactions and subtracts cash handed over",
  async () => {
    const seed = testDate();
    const today = testDate();
    ...
    assert.deepEqual(data.cardCashTransactions, [
      {
        cardCharged: "110.00",
        cashGiven: "100.00",
        extra: "10.00",
...
serialTest(
  "POST /entry rejects a cash-tip row without a tip amount",
  async () => {
    const { status, data } = await req("POST", "/entry", {
      date: testDate(),
      actual: "500",
...
serialTest(
  "POST /entry records POS card, actual card billing, and the matching reason",
  async () => {
    const date = testDate();
    const { status, data } = await req("POST", "/entry", {
      date,
...
serialTest(
  "POST /entry exposes card discrepancy and overall match status",
  async () => {
    const date = testDate();
    const { status, data } = await req("POST", "/entry", {
      date,
...
serialTest(
  "PATCH /entry refreshes the recorded payment discrepancy reason",
  async () => {
    const date = testDate();
...
serialTest(
  "POST /entry with denominations derives actual from denom",
  async () => {
    const today = testDate();
    const { status, data } = await req("POST", "/entry", {
      date: today,
...
// --- Reconcile tests ---
serialTest(
  "POST /reconcile corrects a confirmed day and re-derives the opening",
  async () => {
    ...
    const rec = await req("POST", "/reconcile", {
      date: today,
      actual: corrected,
    });
    assert.equal(
      rec.status,
      200,
      `reconcile failed: ${JSON.stringify(rec.data)}`,
    );
...
serialTest(
  "POST /reconcile on an unconfirmed day refreshes reconciliation",
  async () => {
    const seed = testDate();
    const today = testDate();
    const BALANCE = 400;
...
// --- History tests ---
serialTest(
  "POST /entry with cashSales raises expected and persists in history",
  async () => {
    const day = testDate();
    ...
    const patched = await req("PATCH", `/entry/${day}`, {
      declared: "note only",
    });
...
serialTest(
  "POST /entry records black as unrung cash IN and raises expected",
  async () => {
    const day = testDate();
    const opening = await currentBooksBalance(day);
...
serialTest(
  "full movement mix balances and takeout becomes tomorrow opening",
  async () => {
    const day1 = testDate();
    const day2 = testDate();
    const { status, data } = await req("POST", "/entry", {
    ...
    assert.equal(
      ...
      520,
      "black raises expected; drop and expense lower it",
    );
...
serialTest(
  "POST /entry/:date/move re-dates an entry without losing data",
  async () => {
    const from = testDate();
    const to = testDate();
    await req("POST", "/entry", {
    ...
    const history = await req("GET", "/history");
    assert.ok(
      !history.data.entries.some((e) => e.date === from),
      "old date must be freed",
    );
...
serialTest(
  "POST /entry/:date/move onto an occupied date returns 409",
  async () => {
    const a = testDate();
    const b = testDate();
...
serialTest(
  "moved first entry keeps first-day baseline derivation",
  async () => {
    ...
    const moved = await req("POST", `/entry/${freshFrom}/move`, {
      date: freshTo,
    });
    assert.equal(moved.status, 200);
    assert.equal(Number(moved.data.actual), 116.2);
    assert.equal(Number(moved.data.black), 56.6);
    ...
    assert.equal(
      Number(atNew.opening),
      Number(moved.data.opening),
...
serialTest(
  "move re-points the carried baseline only when moving its own confirmed day",
  async () => {
    ...
    const movedOther = await req("POST", `/entry/${otherConfirmed}/move`, {
      date: testDate(),
    });
    ...
    const movedBase = await req("POST", `/entry/${baseDay}/move`, {
      date: newBaseHome,
    });
    ...
    const probeEntry = await req("POST", "/entry", {
      date: probe,
      actual: "300",
    });
    assert.equal(
      Number(probeEntry.data.opening),
      300,
...
serialTest(
  "PATCH /entry/:date rejects non-object denominations and preserves count",
  async () => {
    const day = testDate();
    await req("POST", "/entry", { date: day, actual: "100" });
    const patched = await req("PATCH", `/entry/${day}`, {
      denominations: "six notes",
    });
...
serialTest(
  "first-day derivation: no opening known -> day balances against its own numbers",
  async () => {
    const day = testDate();
    ...
    assert.equal(
      Number(data.actual),
      150,
      "omitted count must preserve the stored count",
    );
...
serialTest(
  "POST /reconcile folds a corrected count into the carried baseline only for its own day",
  async () => {
    ...
    assert.equal(
      Number(next.data.opening),
      400,
      "baseline after confirm = actual - takeout",
    );
    ...
    assert.equal(
      Number(rec.data.openingCash),
      450,
      "corrected count folded into the baseline",
    );
    const probeA = testDate();
    const probeAEntry = await req("POST", "/entry", {
      date: probeA,
      actual: "450",
    });
    assert.equal(
      Number(probeAEntry.data.opening),
      450,
    ...
    const recTakeout = await req("POST", "/reconcile", {
      date: d3,
      actual: "450",
      takeout: "80",
    });
    ...
    assert.equal(
      Number(recTakeout.data.openingCash),
      370,
    ...
    assert.equal(
      Number(recD1.data.openingCash),
      370,
      "reconciling an older day must leave the carried baseline alone",
    );
    const probeB = testDate();
    const probeBEntry = await req("POST", "/entry", {
      date: probeB,
      actual: "370",
    });
    assert.equal(
      Number(probeBEntry.data.opening),
      450,
...
serialTest(
  "DELETE /entry/:date for non-existent date returns 404",
  async () => {
...
serialTest(
  "POST /entry treats a till expense as accounted cash removed",
  async () => {
    const day1 = testDate();
    const day2 = testDate();
...
serialTest(
  "POST /entry updates the same dated expense without duplicating the day",
  async () => {
    const date = testDate();
    ...
    assert.equal(
      history.data.entries.filter((entry) => entry.date === date).length,
      1,
    );
  },
);
...
serialTest(
  "POST /entry uses the previous confirmed opening for a backdated day",
  async () => {
    const day1 = testDate();
    const day2 = testDate();
    const day3 = testDate();
...
serialTest(
  "POST /confirm keeps a newer carried opening when confirming a backdated day",
  async () => {
...
serialTest(
  "POST /entry on a previously confirmed date upserts instead of failing",
  async () => {
...
serialTest(
  "POST /reconcile corrects takeout and re-derives the carried opening",
  async () => {
...
serialTest(
  "PATCH /entry/:date preserves stored amounts when only the note changes",
  async () => {
    const day1 = testDate();
    const day2 = testDate();
    ...
    const patched = await req("PATCH", `/entry/${day2}`, {
      declared: "ice run",
    });
    const history = await req("GET", "/history");
...
serialTest(
  "PATCH /entry/:date updates takeout on edit (was silently dropped)",
  async () => {
    const day = testDate();
...
serialTest(
  "DELETE /entry/:date keeps the newest remaining saved count as the opening source",
  async () => {
    const day1 = testDate();
    const day2 = testDate();
    const day3 = testDate();
    ...
    const probe = await req("POST", "/entry", {
      date: probeDate,
      actual: "450",
    });
...
serialTest(
  "Served page exposes the date picker, expense field, and API wiring",
  async () => {
    ...
    assert.ok(
      html.includes("Count the drawer"),
      "page must explain the counting workflow",
    );
    assert.ok(
      html.includes("Save &amp; check"),
      "save action must explain its result",
    );
    assert.ok(
      html.includes("app.js"),
      "behavior must load from the CSP-safe external script",
    );
    const script = await fetch(pageUrl + "app.js");
    assert.equal(script.status, 200);
    assert.ok(
      (await script.text()).includes("async function save"),
      "save workflow must be served",
    );
...
serialTest("POST /entry with a malformed amount returns 400", async () => {
  ...
  const hist = await req("GET", "/history");
  assert.ok(
    !hist.data.entries.some((e) => e.date === day),
    "rejected entry must not be stored",
  );
});

serialTest(
  "POST /entry with malformed movement fields returns 400",
  async () => {
    for (const field of [
      "expense",
      "cashAdded",
      ...
      const day = testDate();
      const res = await req("POST", "/entry", {
        date: day,
        actual: "100",
        [field]: "12.345",
      });
      assert.equal(
        res.status,
        400,
        `${field}='12.345' must be rejected with 400`,
      );
...
serialTest(
  "PATCH /entry/:date with a malformed amount returns 400 and keeps the stored row",
  async () => {
    const day = testDate();
    await req("POST", "/entry", { date: day, actual: "100" });
    ...
    assert.equal(
      ...
      100,
      "failed PATCH must not corrupt the stored entry",
    );
  },
);
...
serialTest("POST /reconcile with a malformed amount returns 400", async () => {
  const day = testDate();
  await req("POST", "/entry", { date: day, actual: "100" });
  const res = await req("POST", "/reconcile", {
    date: day,
    actual: "not-a-number",
  });
  assert.equal(res.status, 400);
...
serialTest(
  "POST /api/opening with a malformed amount returns 400",
  async () => {
...
serialTest(
  "POST /entry/:date/move to the same date is a no-op success",
  async () => {
    const day = testDate();
...
serialTest(
  "POST /entry/:date/move for a non-existent date returns 404",
  async () => {
    const res = await req("POST", "/entry/2001-01-01/move", {
      date: testDate(),
    });
...
serialTest(
  "POST /entry/:date/move without a target date returns 400",
  async () => {
    const day = testDate();
...
serialTest(
  "PATCH /entry/:date for a non-existent date returns 404",
  async () => {
    const { status } = await req("PATCH", "/entry/2001-01-01", {
      declared: "x",
    });
    assert.equal(status, 404);
  },
...
serialTest(
  "POST /confirm with a fresh takeout re-derives the carried opening",
  async () => {
    ...
    const day2 = testDate();
    await req("POST", "/entry", { date: day1, actual: "500", opening: "0" });
    const confirmed = await req("POST", "/confirm", {
      date: day1,
      takeout: "120",
    });
    assert.equal(confirmed.status, 200);
    ...
    assert.equal(
      Number(confirmed.data.openingCash),
      380,
      "opening = actual - fresh takeout",
    );
    ...
    assert.equal(
      Number(next.data.opening),
      380,
      "next day opens with the corrected remainder",
    );
  },
);
